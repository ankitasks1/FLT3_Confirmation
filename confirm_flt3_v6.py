...
def parse_arguments():
    ...
    parser.add_argument('--lspan', help='number of bases to cover inside and outside from left sides of ref and alt allele')
    parser.add_argument('--rspan', help='number of bases to cover inside and outside from right sides of ref and alt allele')
    ...
    parser.add_argument('--itdetect', help='whether to run itdetect or not,  y (Run) or n (Do not Run)',  default='none')
    ...
    parser.add_argument('--annotation', help='y=Yes, n=No', default='n')
...
def prep_regions(var_regions_list1, lspan, rspan, id, out_file, in_file):
    count = 1
    out_files_list = []
    in_files_list = []
    ...
    for c in var_regions_list1:
        ...
        if int(lspan) > 0:
            for l_span in range(0,(int(lspan)+1)):
                # print(n_span)
                region_out.append("".join(chrm +'\t' + str(start-l_span)))
                region_ins.append("".join(chrm +'\t' + str(start+l_span)))
        else:
            pass
        if int(rspan) > 0:
            for r_span in range(0,(int(rspan)+1)):
                # print(n_span)
                region_out.append("".join(chrm +'\t' + str(end+r_span)))
                region_ins.append("".join(chrm +'\t' + str(end-r_span)))
        else:
            pass
...
def main():
    ...
    var_regions_files = prep_regions(vcf_info[0], args.lspan, args.rspan, args.id, f"region_out_{args.id}", f"region_in_{args.id}")
    ...
    elif args.annotation.lower() == 'n':
        print('annotation is off ...')
        pass
